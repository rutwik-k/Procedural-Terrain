internal GLuint CreateOpenGLShader(char *vertex_source_file, char *fragment_source_file){
    char *vertex_source = (char *)app_platform->PlatformReadEntireFileToString(vertex_source_file);
    
    char *fragment_source = (char *) app_platform->PlatformReadEntireFileToString(fragment_source_file);
    ...
    if(!success_v){
        ...
        app_platform->PlatformDisplayError(vertex_source_file, info_v);
    }
    ...
    if(!success_f){
        ...
        app_platform->PlatformDisplayError(fragment_source_file, info_f);
    }
    ...
    if(!success_p){
        ...
        app_platform->PlatformDisplayError("Shader Program - Linking error", info_p);
    }
